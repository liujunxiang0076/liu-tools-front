...
### 开发工具 (Development)
- **📄 JSON格式化工具** - JSON数据格式化、压缩、验证和对比，支持树形视图、语法高亮、错误提示和差异对比
- **❄️ 雪花ID生成器** - 生成全局唯一的雪花ID，支持批量生成、自定义配置和ID解析，便于复制到数据库表中
...
- **🌐 URL编码解码** - URL编码、解码和参数解析，支持查询字符串构建
- **🔢 进制转换器** - 二进制、八进制、十进制、十六进制互转，实时验证
- **🔐 JWT解析器** - JWT Token解析和验证，查看Header、Payload和签名信息
- **🔐 哈希计算器** - 支持MD5、SHA-1、SHA-256、SHA-512，文本和文件哈希
- **🌐 User Agent解析器** - 解析UA字符串，识别浏览器、操作系统、设备类型
- **⏰ Cron表达式生成器** - 可视化生成Cron表达式，支持常用定时任务配置

### 设计工具 (Design)
- **🎨 颜色选择器** - 专业的颜色选择和调色板工具，支持HEX、RGB、HSL、HSV格式互转
- **🎨 CSS渐变生成器** - 可视化生成CSS渐变，支持线性、径向、圆锥渐变
- **🎨 CSS阴影生成器** - 可视化生成box-shadow，支持多层阴影和内阴影

### 效率工具 (Efficiency)
- **🔑 密码生成器** - 生成安全的随机密码，支持自定义长度和字符类型，实时强度分析
- **📱 二维码生成器** - 快速生成各种类型的二维码（文本、URL、WiFi、联系人、电话、短信），支持自定义样式、尺寸和颜色
- **🆔 UUID生成器** - 生成全局唯一标识符，支持v1/v4版本，批量生成
- **🔤 大小写转换** - 文本大小写格式转换，支持8种命名规范（驼峰、蛇形、短横线等）
- **📊 文本统计分析** - 统计文本字数、词数、字符等，支持中英文分析和阅读时间估算
- **📝 文本差异对比** - 比较两段文本的差异，高亮显示不同之处，支持逐行对比
- **📏 单位转换器** - 各种单位互转，支持长度、重量、温度、数据大小等
- **📅 日期计算器** - 日期加减、日期差计算、年龄计算和工作日统计

### 🎯 工具特色功能

#### JSON格式化工具
- **树形视图** - 可折叠的树形结构，支持大型JSON数据
- **语法高亮** - 键名、字符串、数字、布尔值等不同颜色显示
- **格式化/压缩** - 一键美化或压缩JSON数据
- **错误提示** - 实时语法检查和错误定位
- **数据统计** - 文件大小、键数量、嵌套深度等信息
- **差异对比** - 支持两个JSON的深度对比，高亮显示差异
- **格式化/还原** - 对比模式下支持格式化和还原原始数据

#### 雪花ID生成器
- **批量生成** - 支持一次生成多个唯一ID
- **自定义配置** - 可设置数据中心ID和机器ID
- **ID解析** - 解析雪花ID的时间戳、数据中心、机器等信息
- **一键复制** - 快速复制单个或全部ID
- **格式化显示** - 清晰展示ID的各个组成部分
...
│   │   ├── JsonTreeNode.vue     # JSON树形节点组件
│   │   └── jsonTreeTypes.ts     # JSON树形数据类型
│   ├── layouts/            # 布局组件
│   │   └── MainLayout.vue       # 主布局
│   ├── views/              # 工具页面
│   │   ├── JsonFormatter.vue    # JSON格式化
│   │   ├── SnowflakeGenerator.vue # 雪花ID生成器
